...
if [ ! -d "$RES" ]; then
  ...
  echo "  engine/tk_SurroundPanner.jsfx, tk_SurroundNoise.jsfx and tk_SurroundMonitor.jsfx into its Effects folder by hand."
  echo
...
if cp engine/tk_SurroundPanner.jsfx engine/tk_SurroundNoise.jsfx engine/tk_SurroundMonitor.jsfx "$RES/Effects/tk/"; then
  echo "  ✓ Installed  JS: tk SurroundPanner + tk SurroundNoise + tk SurroundMonitor  (v0.27.0)  →  Effects/tk/"
  echo
  echo "  In REAPER:"
